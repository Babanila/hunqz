...
describe('InfoItem', () => {
  it('renders the label correctly', () => {
    render(<InfoItem label="Age" value="28" />);

    expect(screen.getByText('Age')).toBeInTheDocument();
  });

  it('renders the value correctly', () => {
    render(<InfoItem label="Location" value="Berlin" />);

    expect(screen.getByText('Berlin')).toBeInTheDocument();
  });

  it('applies the correct classes to label and value', () => {
    render(<InfoItem label="Weight" value="75kg" />);
  ...
  it('renders ReactNode values correctly', () => {
    render(<InfoItem label="Status" value={<span data-testid="status-badge">Online</span>} />);
  ...
  it('renders the container with correct class', () => {
    const { container } = render(<InfoItem label="Height" value="180 cm" />);
